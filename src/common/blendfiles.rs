...
use common::path::{get_blendfilespath, get_data_blendfilespath};
use std::fs;
...
pub fn create_completely_random_job<S>(source_path: S) -> Job where S: Into<String>{
    ...
    create_job_from_blendfile(source_path, id, email, animation)
}
...
pub fn create_random_still_job<S>(source_path: S) -> Job where S: Into<String>{
    ...
    let email = rng.choose(&emails).unwrap().to_string();
    let id = random_id();
    create_job_from_blendfile(source_path, id, email, false)
}
...
pub fn create_random_animation_job<S>(source_path: S) -> Job where S: Into<String>{
    ...
    let email = rng.choose(&emails).unwrap().to_string();
    let id = random_id();
    create_job_from_blendfile(source_path, id, email, true)
}
...
/// `data/blendfiles/<id>`
pub fn create_job_from_blendfile<P, S>(source_path: P, id: S, email: S, animation: bool) -> Job 
